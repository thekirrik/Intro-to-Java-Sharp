...
namespace Exercise_11 {
	class Exercise11 {
		static void Main(string[] args) {
			...
			double secondsInYear = 60d * 60d * 24d * 365d;
			
			// base population we start with
			double population 		= 312032486d;
			
			// amount of births each year
			double birthRate 		= secondsInYear / 7d;
			
			// amount of deaths each year
			double deathRate 		= secondsInYear / 13d;
			
			// amount of immigrations each year
			double immigrationRate 	= secondsInYear / 45d;

			Console.WriteLine("Year 0: " + population);
			for (double i = 1; i < 6; i++) {
				Console.WriteLine("Year " + i + ": " + (population + i * birthRate - i * deathRate + i * immigrationRate));
			}
		}
	}
}